...
import 'package:flutter_audio_recorder/flutter_audio_recorder.dart' as prefix;
import 'package:flutter_ui/presentation/components/chat/constant.dart';
...
class _RecordButtonState extends State<RecordButton> {
  prefix.FlutterAudioRecorder _recorder;
  prefix.Recording _current;
  prefix.RecordingStatus _currentStatus = prefix.RecordingStatus.Unset;
//  Recording _recording = new Recording();
    ...
    if (Platform.isIOS) {
      ...
      try {
        ...
        new Timer.periodic(tick, (Timer t) async {
          if (_currentStatus == prefix.RecordingStatus.Stopped) {
            t.cancel();
  ...
  completeRecord() async {
    if (Platform.isIOS) {
      ...
      if (intervalTime < MIN_INTERVAL_TIME) {
        ...
        widget.onAudioCallBack?.call(file, result.duration.inSeconds);
      }
      _init();
    } else if (Platform.isAndroid) {
      print("安卓");
  ...
  _init() async {
    try {
      if (await prefix.FlutterAudioRecorder.hasPermissions) {
...
//        io.Directory appDocDirectory = await getApplicationDocumentsDirectory();
        ...
        _recorder = prefix.FlutterAudioRecorder(customPath,
            audioFormat: prefix.AudioFormat.WAV);
